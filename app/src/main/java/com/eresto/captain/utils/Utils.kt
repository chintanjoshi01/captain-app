...
class Utils {
    ...
    companion object {
        ...
        fun getCheckedIns(text: String, chips: ChipGroup): String {
            ...
            for (i in 0 until chips.childCount) {
                ...
                if (chip.isChecked) {
                    ...
                    if (arrStr.contains(chip.text.toString())) arrStr.remove(chip.text.toString())
        ...
        fun addChips(
            ...
            if (!sp.isNullOrEmpty() && sp != ("null") && sp != ("[]")) {
                val spI = sp.replace("null,", "").replace(",null", "")
                val items = JSONArray(spI)

                txtTitle.text = activity.resources.getString(R.string.select_special_instruction)
                for (i in 0 until items.length()) {
                    val str = items.getString(i)
                    if (str.isNotEmpty() && str != "null") {
                        ...
                        if (!notes.isNullOrEmpty()) {
                            if (notes.indexOf(str) >= 0) chip.isChecked = true
                        }
                    }
                }
                if (!notes.isNullOrEmpty()) {
